...
Release:    1%{?dist}
Summary:    Highly configurable, framework window manager for X. Fast, light and extensible. Built with luajit
...
VCS:        {{{ git_vcs }}}
Source0:    {{{ git_pack }}}
Patch0:     awesome-master-use-vi-instead-of-nano.patch
...
BuildRequires:  luajit-lgi >= 0.8.0
Requires:   luajit-lgi >= 0.8.0
...
%prep
{{{ git_setup_macro }}}
%patch0 -p1 -b .use-vi-instead-of-nano

# fix shebang
sed -i -e '1 s,^#!.*,#!%{_bindir}/bash,' utils/awesome-client

sed -i 's/COMMAND lua\b/COMMAND luajit/' awesomeConfig.cmake \
    tests/examples/CMakeLists.txt
sed -i 's/LUA_COV_RUNNER lua\b/LUA_COV_RUNNER luajit/' \
    tests/examples/CMakeLists.txt
